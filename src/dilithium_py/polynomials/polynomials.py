...
class PolynomialDilithium(Polynomial):
    ...
    # NEW METHOD TO DO CHANGES TO EACH AND EVERY COEFF OF TYPE CLASS PolynomialDilithium
    def update_polynomial_coefficients(poly):
        updated_coeffs = [round(c / 2**5) for c in poly.coeffs]
        updated_poly = poly.parent(updated_coeffs)
        return updated_poly
    
    def calculate_polynomial_hamming_weight(poly):
        hw = 0
